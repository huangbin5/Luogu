...
struct PN {
    int lev;
    Node *v, *p;
    PN(int _lev, Node* _v, Node* _p) : lev(_lev), v(_v), p(_p) {}
};
...
void print(int i) {
    ...
    printf("line %d:\n", i);
    queue<PN> q;
    q.push(PN(1, root, NULL));
    int lev = 0;
    while (!q.empty()) {
        PN r = q.front();
        q.pop();
        if (r.v) {
            lev == r.lev ? printf("\t%d\t%d\t%d\n", r.p ? r.p->data : -1, r.v->data, r.v->size)
                         : printf("%d:\t%d\t%d\t%d\n", lev = r.lev, r.p ? r.p->data : -1, r.v->data, r.v->size);
            q.push(PN(r.lev + 1, r.v->lc, r.v));
            q.push(PN(r.lev + 1, r.v->rc, r.v));
        }
    }
    fflush(stdout);
}
